...
(
  ...
  if command -v sha256sum >/dev/null 2>&1; then
    sha256sum "$packed" "dsh-otel-plugin-v$version.tar.gz" "dsh-otel-plugin.tar.gz" > SHA256SUMS
  else
    shasum -a 256 "$packed" "dsh-otel-plugin-v$version.tar.gz" "dsh-otel-plugin.tar.gz" > SHA256SUMS
  fi
)
